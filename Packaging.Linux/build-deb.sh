...
# Framework-dependent publish keeps package size reasonable.
echo "Publishing App.Avalonia..."
dotnet publish "$ROOT_DIR/App.Avalonia" -r "$RID" -c Release -o "$PKG_DIR/usr/lib/coder-desktop/app"

# Build the VPN service
echo "Publishing Vpn.Service..."
dotnet publish "$ROOT_DIR/Vpn.Service" -r "$RID" -c Release -o "$PKG_DIR/usr/lib/coder-desktop/service"
...
# Launchers/wrappers
cat > "$PKG_DIR/usr/bin/coder-desktop" <<'WRAPPER'
#!/bin/bash
exec "/usr/lib/coder-desktop/app/Coder Desktop" "$@"
WRAPPER
chmod 755 "$PKG_DIR/usr/bin/coder-desktop"
...
cp "$ROOT_DIR/App.Avalonia/Assets/coder.ico" "$PKG_DIR/usr/share/pixmaps/coder-desktop.ico"

# Default config
cat > "$PKG_DIR/etc/coder-desktop/config.json" <<'CONFIG'
{
  "Manager": {
...
Description: Coder Desktop - Connect to Coder workspaces
 Provides a VPN tunnel service and Linux desktop tray application
 for connecting to Coder workspaces.
